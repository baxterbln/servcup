...
class Letsencrypt
{
    ...
    public function signDomains(array $domains, $reuseCsr = false)
    {
        ...
        foreach ($domains as $domain) {
            ...
            $header = array(
                ...
                "e" => Base64_encoder::encode($accountKeyDetails["rsa"]["e"]),
                "kty" => "RSA",
                "n" => Base64_encoder::encode($accountKeyDetails["rsa"]["n"])

            );
            $payload = $challenge['token'] . '.' . Base64_encoder::encode(hash('sha256', json_encode($header), true));
    ...
    public function revoke_certificate($domain)
    {
        ...
        $cert = Base64_encoder::encode_revoke(base64_decode($pem));
...
keyUsage = nonRepudiation, digitalSignature, keyEncipherment');
    ...
    private function get_csr_content($csrPath) {
        ...
        return trim(Base64_encoder::encode(base64_decode($matches[1])));
    }
    ...
    private function signed_request($uri, array $payload)
    {
        ...
        $header = array(
            ...
            "jwk" => array(
                "kty" => "RSA",
                "n" => Base64_encoder::encode($details["rsa"]["n"]),
                "e" => Base64_encoder::encode($details["rsa"]["e"]),
            )
        );
        ...
        $payload64 = Base64_encoder::encode(str_replace('\\/', '/', json_encode($payload)));
        $protected64 = Base64_encoder::encode(json_encode($protected));

        openssl_sign($protected64.'.'.$payload64, $signed, $privateKey, "SHA256");

        $signed64 = Base64_encoder::encode($signed);
